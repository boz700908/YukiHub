...
#include "stdlib.h"
 ...
 // ★ CG 速度修复：帧率上限（单位 ms），默认 0（关闭）
 int g_frame_cap_ms = 16;
...
#include "ScriptSelector.h"
 ...
 void optionHelp()
 {
    ...
    printf( "      --frame-cap:N\tCG animation frame cap (ms), default=0 (off)\n");
...
void parseOption(int argc, char *argv[]) {
    while (argc > 0) {
        if ( argv[0][0] == '-' ){
            ...
            else if (!strncmp(argv[0]+1, "-frame-cap:", 11)){
                // 与 --debug:1 同款：值内联在同一个 argv 里（--frame-cap:16）
                g_frame_cap_ms = atoi(argv[0]+12);
            }
