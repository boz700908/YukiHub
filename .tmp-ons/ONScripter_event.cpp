...
// ★ CG 速度修复：帧率上限（ms），定义在 onscripter_main.cpp，0 = 关闭
//   timerEvent() 用它给「无等待时的脚本循环」加一个最小帧间隔，
//   否则脚本会以 CPU 全速空转，动态 CG 表现成快进。
extern int g_frame_cap_ms;
...
void ONScripter::timerEvent(bool init_flag)
{
    ...
    // ★ 修复：原版在这里「无条件立刻再跑一轮」，导致没有等待时脚本循环以 CPU 全速运行
    //（表现：动态 CG 比正常快数倍）。改为经由 timerCallback 延迟一个帧间隔再继续。
    if (remaining_time == 0 && !init_flag) {
        if (g_frame_cap_ms > 0) {
            if (timer_id) SDL_RemoveTimer(timer_id);
            timer_id = SDL_AddTimer(g_frame_cap_ms, timerCallback, NULL);
            return;
        }
        SDL_Event event;
        event.type = ONS_BREAK_EVENT;
    ...
    if (event_mode & WAIT_TIMER_MODE){
        ...
        // ★ 核心修复：强制最小帧间隔（防止脚本循环无限制快速运行，导致 CG 鬼畜快）
        if (duration < 0) duration = 0;
        // 如果启用了帧率上限（g_frame_cap_ms > 0），则确保 minimum 帧间隔
        if (g_frame_cap_ms > 0 && duration < g_frame_cap_ms) duration = g_frame_cap_ms;
    }
